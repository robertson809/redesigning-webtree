...
student_requests, students_by_class, courses, course_major, student_major = read_file('fall-2013.csv')
...
def weight(student, class_crn):
    ...
    for four_classes in ranked[student]:
        if class_crn in four_classes:
            ...
            score += ((len(ranked[student]) - ranked[student].index(four_classes)) * i)
            break
    if not found:
        score -= 10000
